...
namespace
{
    ...
    struct advec_u_body
    {
                ...
                const TF* __restrict__ rhoref_rcp=nullptr)
        {
            ...
            double u_interp0 = interp2(u[ijk        ], u[ijk+ii1]);
            double u_interp1 = interp2(u[ijk-ii1    ], u[ijk    ]);

            ut[ijk] +=
                // u*du/dx
                ((
                  - u_interp0 * interp6_ws(u[ijk-ii2], u[ijk-ii1], u[ijk    ], u[ijk+ii1], u[ijk+ii2], u[ijk+ii3])
                  + fabs(u_interp0) * interp5_ws(u[ijk-ii2], u[ijk-ii1], u[ijk    ], u[ijk+ii1], u[ijk+ii2], u[ijk+ii3])
                ) - (
                  - u_interp1 * interp6_ws(u[ijk-ii3], u[ijk-ii2], u[ijk-ii1], u[ijk    ], u[ijk+ii1], u[ijk+ii2])
                  + fabs(u_interp1) * interp5_ws(u[ijk-ii3], u[ijk-ii2], u[ijk-ii1], u[ijk    ], u[ijk+ii1], u[ijk+ii2])
               )) * dxi;

            double v_interp0 = interp2(v[ijk-ii1+jj1], v[ijk+jj1]);
            double v_interp1 = interp2(v[ijk-ii1    ], v[ijk    ]);

            ut[ijk] +=
                // v*du/dy
                ((
                  - v_interp0 * interp6_ws(u[ijk-jj2], u[ijk-jj1], u[ijk    ], u[ijk+jj1], u[ijk+jj2], u[ijk+jj3])
                  + fabs(v_interp0) * interp5_ws(u[ijk-jj2], u[ijk-jj1], u[ijk    ], u[ijk+jj1], u[ijk+jj2], u[ijk+jj3])
                ) - (
                  - v_interp1 * interp6_ws(u[ijk-jj3], u[ijk-jj2], u[ijk-jj1], u[ijk    ], u[ijk+jj1], u[ijk+jj2])
                  + fabs(v_interp1) * interp5_ws(u[ijk-jj3], u[ijk-jj2], u[ijk-jj1], u[ijk    ], u[ijk+jj1], u[ijk+jj2])
                )) * dyi;

            double w_interp0 = interp2(w[ijk-ii1+kk1], w[ijk+kk1]);
            double w_interp1 = interp2(w[ijk-ii1    ], w[ijk    ]);

            if (level.distance_to_start() == 0)
            {
                // w*du/dz -> second order interpolation for fluxtop, fluxbot = 0. as w=0
                ut[ijk] +=
                    - ( rhorefh[k+1] * w_interp0 * interp2(u[ijk    ], u[ijk+kk1]) ) / rhoref[k] * dzi[k];
            }
            else if (level.distance_to_start() == 1)
            {
                ut[ijk] +=
                    // w*du/dz -> second order interpolation for fluxbot, fourth order for fluxtop
                    - ( rhorefh[k+1] * w_interp0 * interp4_ws(u[ijk-kk1], u[ijk    ], u[ijk+kk1], u[ijk+kk2])
                      - rhorefh[k  ] * w_interp1 * interp2(   u[ijk-kk1], u[ijk    ]) ) / rhoref[k] * dzi[k]

                    + ( rhorefh[k+1] * fabs(w_interp0) * interp3_ws(u[ijk-kk1], u[ijk    ], u[ijk+kk1], u[ijk+kk2]) ) / rhoref[k] * dzi[k];
            }
            else if (level.distance_to_start() == 2)
            {
                ut[ijk] +=
                    // w*du/dz -> fourth order interpolation for fluxbot
                    - ( rhorefh[k+1] * w_interp0 * interp6_ws(u[ijk-kk2], u[ijk-kk1], u[ijk    ], u[ijk+kk1], u[ijk+kk2], u[ijk+kk3])
                      - rhorefh[k  ] * w_interp1 * interp4_ws(u[ijk-kk2], u[ijk-kk1], u[ijk    ], u[ijk+kk1]) ) / rhoref[k] * dzi[k]

                    + ( rhorefh[k+1] * fabs(w_interp0) * interp5_ws(u[ijk-kk2], u[ijk-kk1], u[ijk    ], u[ijk+kk1], u[ijk+kk2], u[ijk+kk3])
                      - rhorefh[k  ] * fabs(w_interp1) * interp3_ws(u[ijk-kk2], u[ijk-kk1], u[ijk    ], u[ijk+kk1]) ) / rhoref[k] * dzi[k];
            }
            else if (level.distance_to_end() == 2)
            {
                ut[ijk] +=
                    ...
                    - ( rhorefh[k+1] * w_interp0 * interp4_ws(u[ijk-kk1   ], u[ijk    ], u[ijk+kk1], u[ijk+kk2])
                      - rhorefh[k  ] * w_interp1 * interp6_ws(u[ijk-kk3], u[ijk-kk2], u[ijk-kk1], u[ijk    ], u[ijk+kk1], u[ijk+kk2]) ) / rhoref[k] * dzi[k]

                    + ( rhorefh[k+1] * fabs(w_interp0) * interp3_ws(u[ijk-kk1], u[ijk    ], u[ijk+kk1], u[ijk+kk2])
                      - rhorefh[k  ] * fabs(w_interp1) * interp5_ws(u[ijk-kk3], u[ijk-kk2], u[ijk-kk1], u[ijk    ], u[ijk+kk1], u[ijk+kk2]) ) / rhoref[k] * dzi[k];
            }
            else if (level.distance_to_end() == 1)
            {
                ut[ijk] +=
                    // w*du/dz -> second order interpolation for fluxtop, fourth order for fluxbot
                    - ( rhorefh[k+1] * w_interp0 * interp2(u[ijk    ], u[ijk+kk1])
                      - rhorefh[k  ] * w_interp1 * interp4_ws(u[ijk-kk2], u[ijk-kk1], u[ijk    ], u[ijk+kk1]) ) / rhoref[k] * dzi[k]
            ...
            else
            {
                ...
                // w*du/dz
                TF factor = (
                                rhorefh[k  ] * w_interp1 * interp6_ws(u[ijk-kk3], u[ijk-kk2], u[ijk-kk1], u[ijk    ], u[ijk+kk1], u[ijk+kk2])
                                - rhorefh[k  ] * fabs(w_interp1) * interp5_ws(u[ijk-kk3], u[ijk-kk2], u[ijk-kk1], u[ijk    ], u[ijk+kk1], u[ijk+kk2])
                            ) - (
                                rhorefh[k+1]  * w_interp0 * interp6_ws(u[ijk-kk2], u[ijk-kk1], u[ijk    ], u[ijk+kk1], u[ijk+kk2], u[ijk+kk3])
                                - rhorefh[k+1] * fabs(w_interp0) * interp5_ws(u[ijk-kk2], u[ijk-kk1], u[ijk    ], u[ijk+kk1], u[ijk+kk2], u[ijk+kk3])
                            );
